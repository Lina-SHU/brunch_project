        ...
        <div class="col-md-6">
          ...
          <div class="bg-secondary p-4 mb-5">
            ...
            <div class="d-grid gap-2">
              <button type="button" class="btn btn-block btn-primary" @click="payConfirm">確認付款</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    payConfirm () {
      this.isLoading = true
      const url = `${process.env.VUE_APP_API}api/${process.env.VUE_APP_PATH}/pay/${this.$route.params.id}`
      this.$http.post(url)
        .then(res => {
          if (res.data.success) {
            this.$swal({
